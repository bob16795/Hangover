...
method drawDebug*(e: UISlider, parentRect: Rect) =
  if not e.isActive: return

  procCall e.UIElement.drawDebug(parentRect)

  let 
    bounds = e.bounds.toRect(parentRect)
    x = bounds.x + bounds.width * e.value

  drawLine(
    newVector2(x, bounds.y),
    newVector2(x, bounds.y + bounds.height), 10, COLOR_YELLOW,
  )